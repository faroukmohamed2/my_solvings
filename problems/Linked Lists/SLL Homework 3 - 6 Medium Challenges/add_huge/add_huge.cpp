...
public:
	...
	bool error_addition(Node* fst, Node* lst)
	{
		while (lst && fst)
		{
			if (lst->data > 10 || lst->data < 0 || fst->data > 10 || fst->data < 0)
				return true;
			lst = lst->next;
			fst = fst->next;
		}
	...
	void add_huge(LinkedList& l)
	{
		Node* cur1 = head;
		Node* cur2 =l.head;
		if (!cur2)
			return;
		if (error_addition(cur1, cur2))
		{
